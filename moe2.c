...
exports GGUF. chats. no dependencies (except OpenMP for speed).

cc moe.c -O3 -lm -lpthread -fopenmp -o moe && ./moe --depth 4

sibling of l.c (actually.llama). born from the Arianna Method ecosystem.
grok's architecture, opus's code, oleg's spite + parallelized.
*/
...
static void tok_init(Tokenizer *tok){memset(tok,0,sizeof(Tokenizer));stoi_init(&tok->stoi);for(int i=0;i<256;i++){char h[8];snprintf(h,8,"0x%02x",i);tok->tokens[tok->vocab_size]=strdup(h);stoi_put(&tok->stoi,h,tok->vocab_size);tok->vocab_size++;}tok->tokens[tok->vocab_size]=strdup("<s>");stoi_put(&tok->stoi,"<s>",tok->vocab_size);tok->bos_id=tok->vocab_size++;tok->tokens[tok->vocab_size]=strdup("</s>");stoi_put(&tok->stoi,"</s>",tok->vocab_size);tok->eos_id=tok->vocab_size++;}
...
static void rmsnorm(float *out, float *x, float *w, int d, float eps) {
    float ss = 0;
...
static void matvec(float *out, float *W, float *x, int r, int co) {
    ...
    for (int i = 0; i < r; i++) {
        ...
        for (int j = 0; j < co; j++) s += row[j] * x[j];
        out[i] = s;
    }
}
...
static void rope_bwd(float *dv, int pos, float *cc, float *sc, int hd) {
    int h = hd / 2, off = pos * h;
    for (int i = 0; i < h; i++) {
        float d0 = dv[i], d1 = dv[i + h];
        dv[i] = d0 * cc[off + i] + d1 * sc[off + i];
        dv[i + h] = -d0 * sc[off + i] + d1 * cc[off + i];
    }
}
...
static void top_k_experts(float *logits, int n, int k, int *idx, float *wts) {
    ...
    for (int ki = 0; ki < k; ki++) {
        ...
        for (int i = 0; i < n; i++) if (!used[i] && logits[i] > bv) { bv = logits[i]; bi = i; }
        idx[ki] = bi; wts[ki] = logits[bi]; used[bi] = 1;
    }
    float mx = wts[0];
    for (int i = 1; i < k; i++) if (wts[i] > mx) mx = wts[i];
    float s = 0;
    for (int i = 0; i < k; i++) { wts[i] = expf(wts[i] - mx); s += wts[i]; }
...
static RunState alloc_run(Config *c) {
    ...
    for (int p = 0; p < c->seq_len; p++) for (int i = 0; i < half; i++) {
        ...
        s.cos_cache[p * half + i] = cosf(ang); s.sin_cache[p * half + i] = sinf(ang);
    }
    s.router_logits = calloc(c->n_experts, sizeof(float)); s.expert_out = calloc(c->dim, sizeof(float));
    return s;
}

static void free_run(RunState *s) {
    free(s->x); free(s->xb); free(s->xb2); free(s->xb3); free(s->hb); free(s->hb2); free(s->hbs); free(s->hb2s);
    free(s->q); free(s->k); free(s->v); free(s->att); free(s->logits);
    free(s->key_cache); free(s->value_cache); free(s->cos_cache); free(s->sin_cache);
    free(s->router_logits); free(s->expert_out);
}
...
static float *forward_token(ModelW *w, Config *c, RunState *s, int token, int pos) {
    ...
    for (int l = 0; l < c->depth; l++) { 
        ...
        memcpy(s->key_cache + co, s->k, kd * sizeof(float)); memcpy(s->value_cache + co, s->v, kd * sizeof(float)); 
        
        #pragma omp parallel for
        for (int h = 0; h < c->n_heads; h++) { 
            int kvh = h / hg; float *qh = s->q + h * hd; float *att = s->att + h * c->seq_len; 
            for (int t = 0; t <= pos; t++) {
                int ko = l * c->seq_len * kd + t * kd + kvh * hd; float dot = 0;
            ...
            for (int t = 0; t <= pos; t++) {
                float a = att[t]; int vo = l * c->seq_len * kd + t * kd + kvh * hd;
                #pragma omp simd
                for (int d = 0; d < hd; d++) xb2h[d] += a * s->value_cache[vo + d];
            } 
        } 
        ...
        int ti[8]; float tw[8]; top_k_experts(s->router_logits, c->n_experts, c->top_k, ti, tw); 
        ...
        if (lw->shared_expert) { 
            ...
            matvec(s->hbs, se->w_gate->data, s->xb, H, D); matvec(s->hb2s, se->w_up->data, s->xb, H, D); 
...
static void free_ts(TrainState *s, Config *c) {
    ...
    free(s->layers);
    free(s->residual); free(s->dr); free(s->dxn);
    free(s->dq); free(s->dk); free(s->dv); free(s->dao); free(s->dfxn);
    free(s->dhb); free(s->dhb2); free(s->deo);
    free(s->cos_c); free(s->sin_c);
    if(s->final_n) free(s->final_n);
...
static void mm_fwd(float *C, float *A, float *B, int M, int N, int K) {
    #pragma omp parallel for
    for (int m = 0; m < M; m++) {
        float *cm = C + m * N, *am = A + m * K;
        for (int n = 0; n < N; n++) {
            float s = 0; float *bn = B + n * K;
            #pragma omp simd reduction(+:s)
...
static void mm_bwd(float *dA, float *dB, float *dC, float *A, float *B, int M, int N, int K) {
    #pragma omp parallel for
    for (int m = 0; m < M; m++) {
        float *dc = dC + m * N, *am = A + m * K;
        for (int n = 0; n < N; n++) {
            float d = dc[n]; if (d == 0) continue;
            float *bn = B + n * K;
            #pragma omp simd
            for (int k = 0; k < K; k++) {
                // Thread safety note: dB needs atomic or thread-local reduction in a real setting, 
                // but we keep it simpler here. For pure OpenMP, you'd need #pragma omp atomic for dB.
                #pragma omp atomic
                dA[m * K + k] += d * bn[k];
                #pragma omp atomic
                dB[n * K + k] += d * am[k];
...
static void rn_fwd(float *o, float *x, float *w, int T, int D, float eps) {
    #pragma omp parallel for
    for (int t = 0; t < T; t++) {
        float *xt = x + t * D, *ot = o + t * D;
        float ss = 0;
        #pragma omp simd reduction(+:ss)
        for (int i = 0; i < D; i++) ss += xt[i] * xt[i];
        float inv = 1.0f / sqrtf(ss / D + eps);
        #pragma omp simd
        for (int i = 0; i < D; i++) ot[i] = xt[i] * inv * w[i];
    }
}
static void rn_bwd(float *dx, float *dw, float *dout, float *x, float *w, int T, int D, float eps) {
    for (int t = 0; t < T; t++) {
        float *xt = x + t * D, *dot_ = dout + t * D, *dxt = dx + t * D;
        float ss = 0; for (int i = 0; i < D; i++) ss += xt[i] * xt[i];
        float var = ss / D + eps; float inv = 1.0f / sqrtf(var);
        float cs = 0; for (int i = 0; i < D; i++) cs += dot_[i] * w[i] * xt[i];
        float c2 = cs / (D * var);
        for (int i = 0; i < D; i++) {
            dxt[i] += (dot_[i] * w[i] - xt[i] * c2) * inv;
            dw[i] += dot_[i] * xt[i] * inv;
        }
    }
}
...
static float train_fwd(ModelW *w, Config *c, TrainState *s, int *tokens, int *targets, int T) {
    ...
    for (int l = 0; l < c->depth; l++) { 
        ...
        for (int h = 0; h < c->n_heads; h++) { 
            int kvh = h / hg; 
            for (int t = 0; t < T; t++) { 
                float *qt = la->q + t * qd + h * hd; float *att = la->attn_sc + (t * c->n_heads + h) * T; 
        ...
        for (int t = 0; t < T; t++) { 
            float *rl = la->router_lg + t * c->n_experts; int *ti = la->top_idx + t * c->top_k; float *tw = la->top_wt + t * c->top_k; 
            top_k_experts(rl, c->n_experts, c->top_k, ti, tw); 
            for (int ki = 0; ki < c->top_k; ki++) { 
                int eI = ti[ki]; float eW = tw[ki]; ExpertW *exp = &lw->experts[eI]; ExpertAct *ea = &la->ea[t * c->top_k + ki]; 
                float *xn_t = la->ffn_xn + t * D; 
                matvec(ea->gate_pre, exp->w_gate->data, xn_t, H, D); matvec(ea->up_pre, exp->w_up->data, xn_t, H, D); 
        ...
        if (c->has_shared && lw->shared_expert) { 
            for (int t = 0; t < T; t++) { 
                float *xn_t = la->ffn_xn + t * D; float *sg = la->sh_gate + t * H, *su = la->sh_up + t * H, *sa = la->sh_act + t * H, *sp = la->sh_proj + t * D; 
                matvec(sg, lw->shared_expert->w_gate->data, xn_t, H, D); matvec(su, lw->shared_expert->w_up->data, xn_t, H, D); 
    ...
    if (c->aux_loss_w > 0 && c->n_experts > 1) { 
        for (int l = 0; l < c->depth; l++) { 
            LayerAct *la = &s->layers[l]; float fi[16] = {0}, pi[16] = {0}; 
            for (int t = 0; t < T; t++) { 
                for (int ki = 0; ki < c->top_k; ki++) fi[la->top_idx[t * c->top_k + ki]] += 1.0f; 
...
static void train_bwd(ModelW *w, Config *c, TrainState *s, int *tokens, int *targets, int T, float **g) {
    ...
    /* d_logits */ float *dl = calloc(T * V, 4); for (int t = 0; t < T; t++) { if (targets[t] < 0) continue; float *lt = s->logits + t * V; float *d = dl + t * V; float mx = lt[0]; for (int j = 1; j < V; j++) if (lt[j] > mx) mx = lt[j]; float se = 0; for (int j = 0; j < V; j++) { d[j] = expf(lt[j] - mx); se += d[j]; } for (int j = 0; j < V; j++) d[j] = (d[j] / se) * inv_n; d[targets[t]] -= inv_n; } 
    /* LM head bwd */ float *dfn = calloc(T * D, 4); mm_bwd(dfn, g[1], dl, s->final_n, w->output->data, T, V, D); 
    /* Final norm bwd */ memset(s->dr, 0, T * D * 4); rn_bwd(s->dr, g[2], dfn, s->residual, w->output_norm->data, T, D, c->norm_eps); free(dfn); free(dl); 
    ...
    for (int l = c->depth - 1; l >= 0; l--) { 
        ...
        /* MoE backward */ float *dmo = calloc(T * D, 4); if (c->double_prenorm && lw->ffn_post_norm) { int pgi = gi + 6 + (c->double_prenorm ? 1 : 0); rn_bwd(dmo, g[pgi], s->dr, la->moe_out, lw->ffn_post_norm->data, T, D, c->norm_eps); } else memcpy(dmo, s->dr, T * D * 4); 
        int egi = gi + 6 + (c->double_prenorm ? 2 : 0) + 1; /* expert grad start */ memset(s->dfxn, 0, T * D * 4); 
        /* Shared expert bwd */ 
        if (c->has_shared && lw->shared_expert) { 
            int sgi = egi + c->n_experts * 3; 
            for (int t = 0; t < T; t++) { 
                float *dm = dmo + t * D, *xn_t = la->ffn_xn + t * D; float *sg = la->sh_gate + t * H, *su = la->sh_up + t * H, *sa = la->sh_act + t * H; float *da = calloc(H, 4); 
                for (int i = 0; i < D; i++) for (int j = 0; j < H; j++) { da[j] += dm[i] * lw->shared_expert->w_down->data[i * H + j]; g[sgi + 2][i * H + j] += dm[i] * sa[j]; } 
                for (int i = 0; i < H; i++) { float gd = c->use_gelu ? gelu_bwd(sg[i]) : silu_bwd(sg[i]); float act = c->use_gelu ? gelu_f(sg[i]) : silu_f(sg[i]); float dg = da[i] * su[i] * gd, du = da[i] * act; for (int j = 0; j < D; j++) { s->dfxn[t * D + j] += dg * lw->shared_expert->w_gate->data[i * D + j] + du * lw->shared_expert->w_up->data[i * D + j]; g[sgi][i * D + j] += dg * xn_t[j]; g[sgi + 1][i * D + j] += du * xn_t[j]; } } free(da); 
            } 
        } 
        /* Routed experts bwd */ 
        for (int t = 0; t < T; t++) { 
            for (int ki = 0; ki < c->top_k; ki++) { 
                int eI = la->top_idx[t * c->top_k + ki]; float eW = la->top_wt[t * c->top_k + ki]; ExpertW *exp = &lw->experts[eI]; ExpertAct *ea = &la->ea[t * c->top_k + ki]; float *dm = dmo + t * D, *xn_t = la->ffn_xn + t * D; int egi2 = egi + eI * 3; float *da = calloc(H, 4); 
                for (int i = 0; i < D; i++) { float dp = eW * dm[i]; for (int j = 0; j < H; j++) { da[j] += dp * exp->w_down->data[i * H + j]; g[egi2 + 2][i * H + j] += dp * ea->act_out[j]; } } 
                for (int i = 0; i < H; i++) { float gp = ea->gate_pre[i], up = ea->up_pre[i]; float gd = c->use_gelu ? gelu_bwd(gp) : silu_bwd(gp); float act = c->use_gelu ? gelu_f(gp) : silu_f(gp); float dg = da[i] * up * gd, du = da[i] * act; for (int j = 0; j < D; j++) { s->dfxn[t * D + j] += dg * exp->w_gate->data[i * D + j] + du * exp->w_up->data[i * D + j]; g[egi2][i * D + j] += dg * xn_t[j]; g[egi2 + 1][i * D + j] += du * xn_t[j]; } } free(da); 
            } 
        } 
        /* Router backward */ 
        int rgi = gi + 6 + (c->double_prenorm ? 2 : 0); 
        for (int t = 0; t < T; t++) { 
            float *dm = dmo + t * D; int *ti = la->top_idx + t * c->top_k; float *tw = la->top_wt + t * c->top_k; float dw[8]; 
            for (int ki = 0; ki < c->top_k; ki++) { ExpertAct *ea = &la->ea[t * c->top_k + ki]; float d = 0; for (int i = 0; i < D; i++) d += dm[i] * ea->proj_out[i]; dw[ki] = d; } 
            float dot_wd = 0; for (int ki = 0; ki < c->top_k; ki++) dot_wd += tw[ki] * dw[ki]; float *xn_t = la->ffn_xn + t * D; 
            for (int ki = 0; ki < c->top_k; ki++) { float dlk = tw[ki] * (dw[ki] - dot_wd); int eI = ti[ki]; for (int j = 0; j < D; j++) { s->dfxn[t * D + j] += dlk * lw->w_router->data[eI * D + j]; g[rgi][eI * D + j] += dlk * xn_t[j]; } } 
        } free(dmo); 
        /* FFN norm bwd */ rn_bwd(s->dr, g[gi + 5], s->dfxn, la->res_aa, lw->ffn_norm->data, T, D, c->norm_eps); 
        /* Attention backward */ float *dap = calloc(T * D, 4); if (c->double_prenorm && lw->attn_post_norm) { rn_bwd(dap, g[gi + 6], s->dr, la->attn_proj, lw->attn_post_norm->data, T, D, c->norm_eps); } else memcpy(dap, s->dr, T * D * 4); memset(s->dao, 0, T * qd * 4); mm_bwd(s->dao, g[gi + 4], dap, la->attn_out, lw->wo->data, T, D, qd); free(dap); 
        memset(s->dq, 0, T * qd * 4); memset(s->dk, 0, T * kv * 4); memset(s->dv, 0, T * kv * 4); 
        for (int h = 0; h < c->n_heads; h++) { int kvh = h / hg; for (int t = 0; t < T; t++) { float *doh = s->dao + t * qd + h * hd; float *att = la->attn_sc + (t * c->n_heads + h) * T; float *da = calloc(T, 4); for (int sp = 0; sp <= t; sp++) { float *vs = la->v + sp * kv + kvh * hd; float d = 0; for (int d2 = 0; d2 < hd; d2++) d += doh[d2] * vs[d2]; da[sp] = d; float a = att[sp]; float *dvs = s->dv + sp * kv + kvh * hd; for (int d2 = 0; d2 < hd; d2++) dvs[d2] += a * doh[d2]; } float dot_ad = 0; for (int sp = 0; sp <= t; sp++) dot_ad += att[sp] * da[sp]; float *qt = la->q + t * qd + h * hd; float *dqt = s->dq + t * qd + h * hd; for (int sp = 0; sp <= t; sp++) { float ds = att[sp] * (da[sp] - dot_ad) * sc; float *ks = la->k + sp * kv + kvh * hd; float *dks = s->dk + sp * kv + kvh * hd; for (int d2 = 0; d2 < hd; d2++) { dqt[d2] += ds * ks[d2]; dks[d2] += ds * qt[d2]; } } free(da); } } 
        ...
        memset(s->dxn, 0, T * D * 4); mm_bwd(s->dxn, g[gi + 1], s->dq, la->xn, lw->wq->data, T, qd, D); mm_bwd(s->dxn, g[gi + 2], s->dk, la->xn, lw->wk->data, T, kv, D); mm_bwd(s->dxn, g[gi + 3], s->dv, la->xn, lw->wv->data, T, kv, D); 
        float *ds = calloc(T * D, 4); memcpy(ds, s->dr, T * D * 4); memset(s->dr, 0, T * D * 4); rn_bwd(s->dr, g[gi], s->dxn, la->inp, lw->attn_norm->data, T, D, c->norm_eps); for (int i = 0; i < T * D; i++) s->dr[i] += ds[i]; free(ds); 
    } 
    /* Embedding bwd */ 
...
static void adam_free(Adam *o) {
    if(!o) return;
    for(int i=0; i<o->np; i++) { free(o->states[i].m); free(o->states[i].v); }
    free(o->states); free(o);
}
...
static void export_gguf(ModelW *w, Config *c) {
    ...
    uint64_t off = 0; wti(f, "token_embd.weight", w->tok_emb, &off); wti(f, "output_norm.weight", w->output_norm, &off); wti(f, "output.weight", w->output, &off); 
    for (int l = 0; l < c->depth; l++) { 
    ...
    for (int l = 0; l < c->depth; l++) { LayerW *lw = &w->layers[l]; WD(lw->attn_norm); WD(lw->wq); WD(lw->wk); WD(lw->wv); WD(lw->wo); WD(lw->ffn_norm); if (c->double_prenorm) { WD(lw->attn_post_norm); WD(lw->ffn_post_norm); } WD(lw->w_router); for (int e = 0; e < c->n_experts; e++) { WD(lw->experts[e].w_gate); WD(lw->experts[e].w_up); WD(lw->experts[e].w_down); } if (c->has_shared) { WD(lw->shared_expert->w_gate); WD(lw->shared_expert->w_up); WD(lw->shared_expert->w_down); } } 
...
int main(int argc, char **argv) {
    ...
    for (int step = 0; step < c.max_steps; step++) {
        ...
        /* Local clipping for stability - specific for MoE router blowups */
        for(int i = 0; i < params.count; i++) {
            float t_gn = 0;
            for(int j = 0; j < params.tensors[i]->size; j++) t_gn += grads[i][j] * grads[i][j];
            t_gn = sqrtf(t_gn);
            if(t_gn > 10.0f) { // Arbitrary local threshold to save router
                float ls = 10.0f / t_gn;
                for(int j = 0; j < params.tensors[i]->size; j++) grads[i][j] *= ls;
            }
        }
